...
import {
	MAX_MODELS, MAX_SOUNDS, MAX_EDICTS, MAX_LIGHTSTYLES,
	...
	STAT_SECRETS, entity_state_t
} from './quakedef.js';
import {
	Con_Printf, Con_DPrintf, SZ_Clear,
...
import {
	...
	entity_t, scoreboard_t, lightstyle_t, packet_entities_t
} from './client.js';
...
import {
	CL_SetServerState, CL_AcknowledgeCommand,
...
export function CL_ParseClientdata(bits) {
	...
	if (cl.stats[STAT_HEALTH] !== i) {
		...
		// Logic for score submission on death
		if (cl.stats[STAT_HEALTH] <= 0) {
			if (walletManager.isConnected && !cl.death_score_submitted) {
				cl.death_score_submitted = true;

				// Calculate effective score: use Frags (MP) or Monsters Killed (SP)
				// svc_updatefrags updates cl.wallet_score, svc_killedmonster updates STAT_MONSTERS
				const effectiveScore = Math.max(cl.wallet_score || 0, cl.stats[STAT_MONSTERS] || 0);

				Con_Printf('Death detected. Health: ' + cl.stats[STAT_HEALTH] + ', Score: ' + effectiveScore + '\n');

				// Only submit if we have a positive score (Contract requires > 0)
				if (effectiveScore > 0) {
					Con_Printf('Submitting score ' + effectiveScore + ' to Omega Network...\n');
					walletManager.submitScore(effectiveScore).catch(e => {
						Con_Printf('Score submission failed: ' + e.message + '\n');
					});
				} else {
					Con_Printf('Score is 0 (or negative), skipping submission.\n');
				}
			}
		} else {
			// Player is alive (respawned or healed), reset flag
			if (cl.death_score_submitted) {
				cl.death_score_submitted = false;
				// Con_Printf('Player respawned, score submission reset.\n');
			}
		}
	...
	while (true) {
		...
		switch (cmd) {
			...
			case svc_updatefrags:
				...
				// Track local player score for wallet submission
				if (i === cl.viewentity - 1) {
					cl.wallet_score = cl.scores[i].frags;
				}
				break;
			...
			case svc_intermission:
				...
				// Auto-submit score on level end
				if (walletManager.isConnected && !cl.wallet_score_submitted) {
					cl.wallet_score_submitted = true;
					walletManager.submitScore(cl.wallet_score).catch(e => {
						Con_Printf('Score submission failed: ' + e.message + '\n');
					});
				}
				break;
			...
			case svc_sellscreen:
				...
				Con_Printf('Ignored sellscreen command.\n');
				break;
